...
from app.zillow_client import (
    search_properties, search_by_address, is_address_search,
    API_CONFIGURED, PROVIDER_NAME, POPULAR_CITIES,
)
...
async def search(location: str = Query(..., min_length=2, description="City, State, ZIP, or full address")):
    ...
    if cached:
        ...
        return SearchResponse(
            ...
            search_type="address" if is_address_search(location) else "area",
        )

    try:
        # Route to address search or area search based on input
        if is_address_search(location):
...
async def health():
    return {
        ...
        "mode": "live" if API_CONFIGURED else "demo",
        "data_source": PROVIDER_NAME,
    }
